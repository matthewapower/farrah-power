...
export default function Shop() {
  ...
  return (
    <Layout message="Photos Worth Keeping*">
      <SEO title="Home" />
      <header className="flex justify-center items-center px-4 md:mt-40 mb-12">
        <div className="px-4 text-center">
          <h2 className="font-heading uppercase text-3xl mb-4 tracking-wide">
            Shop
          </h2>
          <p className="max-w-lg mx-auto mb-6">
            These photos were taken in various locations out west. Documenting
            my travels has always evoked a sense of wonder, inspiration, and
            freedom in me. I hope that these photos can be a reminder of the
            light and natural beauty found in nature.
          </p>
        </div>
      </header>
      <section className="px-4 grid md:grid-cols-2 gap-12 border-b border-black pb-12">
        {data.shopifyCollection.products.map((b, i) => {
          return (
            <Link
              to={`/product/${b.handle}`}
              className="hover:opacity-75 relative bg-tan rounded-lg h-screen-50 md:h-screen-75"
            >
              <h2 className="text-xl md:text-3xl text-center mx-4 mt-4 md:mt-12 font-display">
                {b.title}
              </h2>
              <Img
